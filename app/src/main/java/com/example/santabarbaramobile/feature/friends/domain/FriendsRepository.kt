...
class FriendsRepository @Inject constructor(
    ...
    suspend fun getRelationshipStatus(otherAuthId: String): Result<FriendStatusResponse> =
    ...
    suspend fun sendFriendRequest(receiverAuthId: String): Result<Unit> =
        withContext(Dispatchers.IO) {
            try {
                val token = "Bearer ${tokenManager.getToken()}"
    ...
    suspend fun getFriends(authId: String, page: Int = 1): Result<FriendPaginationResponse> =
        withContext(Dispatchers.IO) {
            try {
                val token = "Bearer ${tokenManager.getToken()}"
    ...
    suspend fun getIncomingRequests(page: Int = 1): Result<FriendRequestPaginationResponse> =
        withContext(Dispatchers.IO) {
            try {
                val token = "Bearer ${tokenManager.getToken()}"
    ...
    suspend fun getOutgoingRequests(page: Int = 1): Result<FriendRequestPaginationResponse> =
        withContext(Dispatchers.IO) {
            try {
                val token = "Bearer ${tokenManager.getToken()}"
